...
// is updated
int ctrlUpdate( Stg::World* world, CStageRobot* robot )
{
  assert( robot );
  robot->updateControllers();
  return 0; // ok
}
...
int CStageRobot::findDevice( ARangeFinder* &device, std::string devName )
{
  ...
  if ( devName.find( LASER_MODEL_NAME ) != std::string::npos ) {
    ...
    laser->updateData( modLaser->GetUpdateInterval() / 1e6 );
    return 1; // success
  }
  ...
  if ( devName.find( RANGER_MODEL_NAME ) != std::string::npos ) {
    ...
    device->setEnabled( true );
    // enforce one update step, to fill the data structures with valid data
    sonar->updateData( modRanger->GetUpdateInterval() / 1e6 );
    return 1; // success
  }
...
int CStageRobot::findDevice( ADrivetrain2dof* &device, std::string devName )
{
  ...
  device->setEnabled( true );
  // enforce one update step, to fill the data structures with valid data
  drivetrain2of->updateData( modPosition->GetUpdateInterval() / 1e6 );
  return 1; // success
}
//-----------------------------------------------------------------------------
int CStageRobot::findDevice( APowerPack* &device, std::string devName )
{
  ...
  device->setEnabled( true );
  // enforce one update step, to fill the data structures with valid data
  powerPack->updateData( modPosition->GetUpdateInterval() / 1e6 );
...
int CStageRobot::findDevice( AFiducialFinder* &device, std::string devName )
{
  ...
  device->setEnabled( true );
  // enforce one update step, to fill the data structures with valid data
  fiducialFinder->updateData( modFiducial->GetUpdateInterval() / 1e6 );
...
//-----------------------------------------------------------------------------
int CStageRobot::findDevice( ASwitchArray* &device, std::string devName )
{
  device = NULL;
  ERROR0( "Device not implementated for stage" );
  return 0;
...
int CStageRobot::findDevice( ATextDisplay* &device, std::string devName )
{
  ...
  device->setEnabled( true );
  // enforce one update step, to fill the data structures with valid data
  textDisplay->updateData( modPosition->GetUpdateInterval() / 1e6 );
